...
<h1><?php echo __('Manage all Pages Consolidated','activitytime'); ?></h1>

<div class="see-wrapper">
    <div class="see-panel see-panel-default">
        <div class="see-panel-heading flex">
            <h3 class="see-panel-title"><?php echo __('All Pages Consolidated','activitytime'); ?></h3>
        </div>
        <div class="see-panel-body">
            ...
            <div class="form-inline">
                <div class="footer-btns">
                    <a href="<?php echo admin_url("admin.php?page=actt_time_per_pageacc&function=export_csv_per_pageacc"); ?>" class="export_csv btn btn-warning pull-right <?php if ( !function_exists('activitytimepro_fms') || !activitytimepro_fms()->is_plan_or_trial('activitytimepropro') ) echo 'actt-pro'; ?>"><i class="fa fa-download"></i>&nbsp;&nbsp;<?php echo __('Export CSV','activitytime')?></a>
...
jQuery(document).ready(function($) {
    ...
    $('[name="filter_user"]').on('change', function(){
        if($(this).val() != '') {
            $('.export_csv').attr('href', '<?php echo get_admin_url() . "admin.php?page=actt_time_per_pageacc&function=export_csv_per_pageacc"; ?>&filter_user='+$(this).val());
        } else {
            $('.export_csv').attr('href', '<?php echo get_admin_url() . "admin.php?page=actt_time_per_pageacc&function=export_csv_per_pageacc"; ?>');
        }
    });*/
    ...
    $('#bulk_remove').click(function(){
        ...
        else
        {
            ...
            if(confirm('<?php echo_js(__('Are you sure?', 'activitytime')); ?>'))
            {
                ...
                // ajax to remove rows
                $.post('<?php menu_page_url( 'actt_time_per_pageacc', true ); ?>&function=bulk_remove', { visited_pages_ids: ids }, function(data) {
